...
export default class GameManager {
  ...
  handleUIGameSetup() {
    ...
    this.boardPlayerDiv.getElementsByClassName("board-info-name")[0].innerHTML =
      TEXT.SETUP_TITLE;
    this.boardPlayerDiv.getElementsByClassName(
      "board-info-notify"
    )[0].innerHTML = TEXT.SETUP_DIRECTION;
    this.boardPlayerDiv
    ...
    const frameDraw = () => {
      ...
      if (now - lastFrame >= CONFIG.TIME_OF_FRAME) {
        if (
          (colorStop > 0 && colorStopDirection > 0) ||
          (colorStop < -delta && colorStopDirection < 0)
        ) {
        ...
        let r = Utilities.ranged(
          colors[this.currentPlane].r + colorStop,
          0,
          255
        );
        let g = Utilities.ranged(
          colors[this.currentPlane].g + colorStop,
          0,
          255
        );
        let b = Utilities.ranged(
          colors[this.currentPlane].b + colorStop,
          0,
          255
        );
        this.boardPlayer.setPlaneInfo(this.currentPlane, {
          color: `rgb(${r}, ${g}, ${b})`,
        });
        lastFrame = now;
      }
  ...
  handleKeyEventGameSetup() {
    ...
    this.keyEventCallback = (e) => {
      this.boardPlayerDiv
        .getElementsByClassName("board-info-notify")[0]
        .classList.remove("highlight-red");
      this.boardPlayerDiv.getElementsByClassName(
        "board-info-notify"
      )[0].innerHTML = TEXT.SETUP_DIRECTION;
      let dx = 0,
      ...
      if (e.keyCode === KEY_EVENT.UP) {
        ...
        if (
          ...
          this.boardPlayerDiv.getElementsByClassName(
            "board-info-notify"
          )[0].innerHTML = TEXT.SETUP_OUTSIDE_BOARD;
          return;
        }
        ...
        if (this.boardPlayer.hasOverlapped()) {
          ...
          this.boardPlayerDiv.getElementsByClassName(
            "board-info-notify"
          )[0].innerHTML = TEXT.SETUP_OVERLAP;
          return;
        }
  ...
  handleUIInGame() {
    ...
    document
      .getElementById("in-game")
      .insertBefore(this.boardPlayerDiv, this.boardOpponentDiv);
    this.boardPlayerDiv.getElementsByClassName("board-info-name")[0].innerHTML =
      player.user.username;
    this.boardPlayerDiv.getElementsByClassName(
      "board-info-notify"
    )[0].innerHTML = `Lives: <span id="user-lives" class="highlight-red">${player.user.lives}</span>`;
    let canvas = document.getElementById("canvas-player");
    ...
    this.boardOpponentDiv = document.getElementById("board-opponent");
    this.boardOpponentDiv.getElementsByClassName(
      "board-info-name"
    )[0].innerHTML = player.opponent.username;
    this.boardOpponentDiv.getElementsByClassName(
      "board-info-notify"
    )[0].innerHTML = `Lives: <span id="opponent-lives" class="highlight-red">${player.opponent.lives}</span>`;
    ...
    const frameDraw = () => {
      ...
      if (now - lastFrame >= CONFIG.TIME_OF_FRAME * 3) {
        if (
          (stop >= delta && stopDirection > 0) ||
          (stop <= 0 && stopDirection < 0)
        ) {
  ...
  handleKeyEventInGame() {
    ...
    this.keyEventCallback = (e) => {
      ...
      if (
        document.activeElement === document.getElementById("chat-box-input")
      ) {
        return;
      }
      let dx = 0,
      ...
      if (e.keyCode === KEY_EVENT.UP) {
        ...
        // ATTACK
        if (
          this.boardOpponent.hasBullet(
            this.boardOpponent.indicator.x,
            this.boardOpponent.indicator.y
          )
        ) {
          return;
        }
        ...
        this.socket.attack({
          x: this.boardOpponent.indicator.x,
          y: this.boardOpponent.indicator.y,
          type: null,
        });
      ...
      if (
        this.boardOpponent.indicator.x + dx < 1 ||
        this.boardOpponent.indicator.x + dx > UI_BOARD.BOARD_SIZE - 2 ||
        this.boardOpponent.indicator.y + dy < 1 ||
        this.boardOpponent.indicator.y + dy > UI_BOARD.BOARD_SIZE - 2
      ) {
        return;
      }
  ...
  updateLives(player) {
    ...
    document.getElementById(`${player}-lives`).innerHTML =
      this.gameConfig.player[player].lives;
  }
  ...
  addBullet(player, bullet) {
    ...
    if (player === "user" && bullet.type === BULLET_TYPE.HEAD) {
      board.planes.find(
        (plane) => plane.headX === bullet.x && plane.headY === bullet.y
      ).dead = true;
    }
  }
  ...
  setChatText(player, text) {
    const boardDiv =
      player === "user" ? this.boardPlayerDiv : this.boardOpponentDiv;
